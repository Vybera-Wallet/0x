...
async function doSwapNative(web3, network, sellAmount, nativeToken, buyTokenName) {
    const deployedAddress = TEST_CONFIG[network].deployedAddress;
    const contract = new web3.eth.Contract(ABIEX, deployedAddress);
    const [owner] = await web3.eth.getAccounts();
...
async function test2(network) {
    ...
    await doSwapNative(web3, network, sellAmount, nativeToken, token1);
    await doSwapNative(web3, network, sellAmount, nativeToken, token2);
